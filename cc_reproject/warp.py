...
def _transform_boundary_pix(src_xy_shape, src_affine, src_crs, dst_crs, res):
    s_offsets, s_endsets = _boundary_from_shape_affine(src_xy_shape, src_affine)
    
    d_offsets, d_endsets = _transform_xy_pts([s_offsets, s_endsets], src_crs, dst_crs)
...
def _calc_default_dst_shape_boundary(src_xy_shape, src_affine, src_crs, dst_crs):
    ...
    (d_xoff, d_yoff), (d_xend, d_yend) = d_offsets, d_endsets = _transform_xy_pts([s_offsets, s_endsets], src_crs, dst_crs)
...
def _block_reproject(src, src_transform, src_crs, dst_shape, dst_transform, dst_crs, kwargs):
...
def _np_reproject(src, src_transform, src_crs, dst_shape, dst_crs, **kwargs):
    ...
    if dst_shape is None:    
        ...
        s_offsets, s_endsets =  _boundary_from_shape_affine(src_xy_shape, src_transform)
        d_offsets, d_endsets = _transform_xy_pts([s_offsets, s_endsets], src_crs, dst_crs)
    ...
    s_affine, s_pix0, s_pix1 = _transform_boundary_pix(dst_xy_shape, d_transform, dst_crs, src_crs, src_res)
    s_x0, s_y0 = s_pix0
    s_x1, s_y1 = s_pix1
    src_crop = _src[:, s_y0:s_y1, s_x0:s_x1]
    reprojected_data = _block_reproject(src_crop, s_affine, src_crs, dst_shape, d_transform, dst_crs, kwargs)
    
    if _src.dtype != src.dtype:
        reprojected_data = reprojected_data.astype(np.uint8)

    reprojected_da = _add_coords_crs_tr_da(reprojected_data, d_bounds, dst_crs, d_transform)
    return reprojected_da


def _dask_reproject(src, src_transform, src_crs, dst_shape, dst_chunks, dst_crs, **kwargs):
    """
    Reprojection with dask by reprojecting chunk by chunk.
    
    Computes the destination chunks based on source chunks or specified chunks, then finds area
    in source raster that gives data for each chunk and then warps the chunks using
    ...
    src_transform: affine transform corresponding to source offsets, res
    ...
    **kwargs: 
    ...
    Returns:
    --------
    xarray DataArray wrapped over a dask array with coords set for dst offsets, res
    """
    if src.dtype == np.uint8:
        _src = src.astype(np.uint16)
    else:
    ...
    if dst_shape is None and dst_chunks is None:    
        ...
        src_offsets, src_endsets = _boundary_from_shape_affine(src_xy_shape, src_transform)
        d_offsets, d_endsets = _transform_xy_pts([src_offsets, src_endsets], src_crs, dst_crs)

    
    if dst_chunks is None:
        numblocks = src.numblocks
        bandchunks = src.chunks[0]
        dst_chunks_hw = _calc_chunks_hw(numblocks[1:], dst_shape[1:])
        dst_chunks = (bandchunks, *dst_chunks_hw)
    else:
        if dst_shape is None:
        ...
        if not isinstance(_src, da.Array):
            chunks = tuple(int(math.ceil(src_shape[i]/numblocks[i])) for i in range(len(numblocks)))
    ...
    dst_chunkshapes = _chunkshapes(dst_chunks, numblocks)
    ...
    sbk_offend_sets = [(sbk_xpnd[i], sbk_xpnd[i + 1]) for i in range(0, len(sbk_xpnd), 2)]
    ...
    sbk_affines_pix = [_sbk_affine_pad_pix(sbk_offend_set, src_res, src_shape, src_transform, padding)
    ...
    def _add_layers(self, locs,
        ...
        for key in keys:
            if isinstance(key, tuple):
                ...
                layers[crop_name][(crop_name, *ii)] = (_crop,
                                                       ...
                                                       s_x0, s_x1,
                                                       b0, b1)
        crop_graph = HighLevelGraph.from_collections(crop_name, layers[crop_name], dependencies=[self])
        arr = da.Array(crop_graph, crop_name, chunks=sbk_chunks, dtype=np.uint16, shape=sbk_tot_shape)
        ...
        for key in arr_keys:
            if isinstance(key, tuple):
                ...
                layers[reproject_name][(reproject_name, *ii)] = (_block_reproject,
                                                                 (input_key, *ii),
                                                                 s_affine,
                                                                 src_crs,
                                                                 dbk_shape,
        ...
        graph = HighLevelGraph.from_collections(reproject_name, layers[reproject_name], dependencies=[arr])
        return da.Array(graph, reproject_name, chunks=dst_chunks, dtype=np.uint16, shape=dst_shape)
    reproject_dsk = _add_layers(_src, d_shp_tr_s_tr_pix_bd,
                                src_crs, sbk_chunks, sbk_tot_shape,
                                dst_shape, dst_chunks, dst_crs, **kwargs)

    if _src.dtype != src.dtype:
        reprojected_dsk = reproject_dsk.astype(np.uint8)

    reprojected_da = _add_coords_crs_tr_da(reproject_dsk, dst_bounds, dst_crs, dst_affine)
    return reprojected_da


def reproject(arr, dst_crs, dst_shape=None, chunks=None, **kwargs):
    """
    Reprojects spatially referenced rasters to desired crs.
    
    Takes in an xarray DataArray raster with source crs x and y coords and apatial ref coord
    and warps the shape to the desired crs, resetting the coords as well. 
    Uses rasterio.warp.reproject. If no destination shape is provided then default shape
    is calculated based on the input and output crs and the starting shape.
    Works with dask and chunking, inluding multiband chunking.
    
    Parameters:
    -----------
    arr: xarray DataArray with crs and transform set in the coords in rioxarray style
    
    dst_crs: str; input crs string; e.g. 'EPSG:4326'
    
    dst_shape(optional): tuple(bands, y, x); destination shape, if known; number of bands
    must match source
    
    chunks(optional): tuple of tuples defining destination chunks for a dask array; for example:
    ((1,), (120, 120), (240, 240)) for a shape chunked 1, 2, 2 along bands, height, width;
    setting this will make reproject return a chunked dask array, even with np input.
    
    **kwargs: additional keyword args, inlcuding padding for dask chunks and any others to be
    passed to rasterio.warp.reproject
        padding: int; number of pixels to pad each src block being reprojected
    
    Returns:
    --------
    arr: xarray DataArray with coords set according to dst crs, including spatial ref coord
    """
    src = arr.data
    src_crs = CRS(arr.spatial_ref.crs_wkt).to_string()
    src_transform = A.from_gdal(*[float(num) for num in arr.spatial_ref.GeoTransform.split(' ')])
    if isinstance(src, da.Array) or chunks is not None:
        reprojected_data = _dask_reproject(src, src_transform, src_crs, dst_shape, chunks, dst_crs, **kwargs)
    elif isinstance(src, np.ndarray):
        reprojected_data = _np_reproject(src, src_transform, src_crs, dst_shape, dst_crs, **kwargs)
