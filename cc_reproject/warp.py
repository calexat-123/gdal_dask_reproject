...
def _dask_reproject(src, src_transform, src_crs,
    ...
    or specified chunks, then finds area in source raster that
    gives data for each chunk and then warps the chunks using
    ...
    **kwargs:
    ...
    Returns:
    --------
    xarray DataArray wrapped over a dask array
    with coords set for dst offsets, res
    """
    if src.dtype == np.uint8:
        _src = src.astype(np.uint16)
    else:
    ...
    def _add_layers(self, locs,
        ...
        crop_graph = HighLevelGraph.from_collections(crop_name,
                                                     layers[crop_name],
                                                     dependencies=[self])
        arr = da.Array(crop_graph, crop_name,
                       chunks=sbk_chunks, dtype=np.uint16, shape=sbk_tot_shape)
        ...
        graph = HighLevelGraph.from_collections(reproject_name,
                                                ...
                                                dependencies=[arr])
        return da.Array(graph, reproject_name,
                        chunks=dst_chunks, dtype=np.uint16, shape=dst_shape)
...
def reproject(arr, dst_crs, dst_shape=None, chunks=None, **kwargs):
    ...
    Takes in an xarray DataArray raster with source crs x and y coords and
    spatial ref coord and warps the shape to the desired crs,
    resetting the coords as well. Uses rasterio.warp.reproject.
    If no destination shape is provided then default shape
    ...
    dst_shape(optional): tuple(bands, y, x); destination shape
    if known; number of bands must match source
    ...
    **kwargs: additional keyword args, inlcuding padding for
    dask chunks and any others to be passed to rasterio.warp.reproject
        - padding: int; number of pixels to pad each
        src block being reprojected

    Returns:
    --------
    arr: xarray DataArray with coords set according to
    dst crs, including spatial ref coord
    """
    src = arr.data
    src_crs = CRS(arr.spatial_ref.crs_wkt).to_string()
    src_transform = A.from_gdal(*[float(num) for num
                                  in arr.spatial_ref.GeoTransform.split(' ')])
    if isinstance(src, da.Array) or chunks is not None:
        reprojected_data = _dask_reproject(src, src_transform, src_crs,
                                           dst_shape, chunks, dst_crs,
                                           **kwargs)
